# Here we use a dadaset previously generated
...
RESULTS_DIRECTORY = ROOT_DIRECTORY / "results_heat_p_10_275"
...
p = 10  # dimension of the trunk net
...
for l_c in l_cor:
    ...
    for n_sensors_x in n_sensors_x_list:
        ...
        #    )
        #    print(
        #        f"""generación completada l {l}
        #                n_sensors_x: {n_sensors_x}, n: {n_funciones[-1]}"""
        #    )
        y_train_long = np.load(ROOT_DIRECTORY / "data" / 'y_train_heat.npy')
        y_test_long = np.load(ROOT_DIRECTORY / "data" / 'y_test_heat.npy')
        X_train_long = np.load(ROOT_DIRECTORY / "data" / 'X_train_heat.npy')
        X_test_long = np.load(ROOT_DIRECTORY / "data" / 'X_test_heat.npy')
